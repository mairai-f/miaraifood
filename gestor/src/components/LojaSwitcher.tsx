...
export function LojaSwitcher({ compact = false }: { compact?: boolean }) {
  ...
  async function confirmExcluirLoja() {
    ...
    try {
      ...
      const r = await fetch(`/api/lojas/${encodeURIComponent(deleteTarget.id)}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${getToken()}` },
      ...
      if (getLojaAtivaId() === deleteTarget.id) setLojaAtivaId("");
      setDeleteTarget(null);
      await carregar();
    } finally {
      setIsDeleting(false);
  ...
  return (
    <div className="relative">
            ...
            >
              ...
              {!l.padrao && (
                <span className="ml-1 flex items-center gap-1">
                  ...
                  <span role="button" tabIndex={0} title="Excluir loja" onClick={(event) => { event.stopPropagation(); setDeleteTarget(l); }} className="rounded p-1 text-rose-300/60 hover:bg-rose-500/10 hover:text-rose-200"><Trash2 size={13} /></span>
                </span>
              )}
            </button>
          ))}
      ...
      <ConfirmModal
        ...
        actionName={`Excluir a Loja "${deleteTarget?.nome}"`}
        description="Esta ação removerá esta loja do painel da conta. O histórico fiscal e contábil será preservado conforme exigências legais."
        confirmText="Continuar e Excluir Loja"
        onConfirm={confirmExcluirLoja}
