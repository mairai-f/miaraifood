...
const SEGMENT_ICONS: Record<string, string> = {
  pizzaria: '🍕', churrascaria: '🔥', restaurante: '🍽️', bar: '🍻', japones: '🍣', 'restaurante-japones': '🍣',
  hamburgueria: '🍔', cafeteria: '☕', padaria: '🥖', aougue: '🥩', hortifrut: '🥬', mercearia: '🛒',
  sorveteria: '🍨', marmitaria: '🍱', pastelaria: '🥟', 'food-truck': '🚚', adega: '🍷',
  'drive-thru-bebidas': '🥤', petshop: '🐾', 'casa-de-show': '🎵', cantina: '🍽️', choperia: '🍺',
  conveniencia: '🛍️', boteco: '🥟', 'cachorro-quente': '🌭', pousada: '🏨', motel: '🛏️', hotel: '🏨', outros: '✨',
};
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {SEGMENTS.map(seg => (
          ...
          >
            <div className="text-2xl leading-none">{seg.icon}</div>
...
export function MarketingPanel() {
  ...
  const [modoCreate, setModoCreate] = useState<'ia' | 'manual'>('ia');
  const [manualForm, setManualForm] = useState({ title: '', headline: '', copy: '', hashtags: '', callToAction: '' });
  const [savingManual, setSavingManual] = useState(false);
  const [editingCampaign, setEditingCampaign] = useState<any>(null);
  const [editForm, setEditForm] = useState({ title: '', headline: '', copy: '', hashtags: '', callToAction: '' });
  ...
  const criarManual = async () => {
    if (!manualForm.title.trim() || !manualForm.copy.trim()) return;
    setSavingManual(true);
    try {
      const response = await fetch('/api/marketing/campaigns/manual', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${getToken()}` },
        body: JSON.stringify({
          ...manualForm,
          hashtags: manualForm.hashtags.split(/[\s,]+/).map(h => h.trim()).filter(Boolean),
        }),
      });
      if (response.ok) {
        setManualForm({ title: '', headline: '', copy: '', hashtags: '', callToAction: '' });
        setTab('history');
        void loadCampaigns();
      }
    } finally {
  ...
  const salvarEdicao = async () => {
    ...
    await fetch(`/api/marketing/campaigns/${editingCampaign.id}`, {
      ...
      body: JSON.stringify({
        ...editForm,
        hashtags: editForm.hashtags.split(/[\s,]+/).map(h => h.trim()).filter(Boolean),
      }),
    });
    ...
    <div className="space-y-4">
      ...
      {tab === 'create' && (
        <div className="space-y-4">
          <div className="flex gap-2">
            <button
              onClick={() => setModoCreate('ia')}
              className={`rounded-lg px-3 py-1.5 text-xs font-medium transition ${modoCreate === 'ia' ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40' : 'bg-slate-800 text-slate-400 border border-transparent'}`}
            >
              ✦ Gerar com IA
            </button>
            <button
              onClick={() => setModoCreate('manual')}
              className={`rounded-lg px-3 py-1.5 text-xs font-medium transition ${modoCreate === 'manual' ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40' : 'bg-slate-800 text-slate-400 border border-transparent'}`}
            >
              ✍️ Criar manualmente
            </button>
          </div>
          ...
          {modoCreate === 'manual' ? (
            <div className="space-y-3 rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
              <div>
                <label className="mb-1 block text-xs text-slate-400">Título da promoção</label>
                <input
              ...
              <div>
                <label className="mb-1 block text-xs text-slate-400">Chamada principal (headline)</label>
                <input
                  value={manualForm.headline}
                  onChange={e => setManualForm({ ...manualForm, headline: e.target.value })}
                  placeholder="Ex: Rodízio com 20% de desconto"
                  className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-100 outline-none focus:border-emerald-500"
                />
              </div>
              <div>
                <label className="mb-1 block text-xs text-slate-400">Texto da campanha</label>
                <textarea
                  value={manualForm.copy}
                  onChange={e => setManualForm({ ...manualForm, copy: e.target.value })}
                  rows={4}
                  placeholder="Escreva o texto completo da sua promoção..."
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <div>
                  <label className="mb-1 block text-xs text-slate-400">Hashtags (separadas por espaço)</label>
                  <input
                    value={manualForm.hashtags}
                    onChange={e => setManualForm({ ...manualForm, hashtags: e.target.value })}
                    placeholder="#promocao #sextafeira"
                    className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-100 outline-none focus:border-emerald-500"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-xs text-slate-400">Chamada para ação</label>
                  <input
                    value={manualForm.callToAction}
                    onChange={e => setManualForm({ ...manualForm, callToAction: e.target.value })}
                    placeholder="Peça já pelo app!"
                    className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-100 outline-none focus:border-emerald-500"
                  />
                </div>
              </div>
              <button
                onClick={criarManual}
                disabled={savingManual || !manualForm.title.trim() || !manualForm.copy.trim()}
                className="rounded-xl bg-emerald-500 px-6 py-2.5 font-medium text-slate-950 hover:bg-emerald-400 transition disabled:opacity-40"
              >
                {savingManual ? 'Salvando...' : 'Salvar promoção'}
              </button>
            </div>
          ) : (
          <>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              >
                ...
                <option value="bar-com-show">Bar / Boteco</option>
                <option value="restaurante-japones">Japonês / Sushi</option>
              ...
              {editingCampaign?.id === c.id ? (
                <div className="space-y-2">
                  <input
                    ...
                    placeholder="Título"
                    className="w-full rounded-lg border border-slate-700 bg-slate-950 px-2.5 py-1.5 text-sm text-slate-100 outline-none focus:border-emerald-500"
                  />
                  <input
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Texto"
                    className="w-full rounded-lg border border-slate-700 bg-slate-950 px-2.5 py-1.5 text-sm text-slate-100 outline-none focus:border-emerald-500 resize-none"
                  />
                  <input
                  ...
                  />
                  <div className="flex gap-2">
                    <button onClick={salvarEdicao} className="rounded-lg bg-emerald-500 px-3 py-1.5 text-xs font-medium text-slate-950 hover:bg-emerald-400">
                      Salvar
                    </button>
                    <button onClick={() => setEditingCampaign(null)} className="rounded-lg bg-slate-800 px-3 py-1.5 text-xs text-slate-300 hover:bg-slate-700">
                      Cancelar
                    </button>
                  </div>
                </div>
                ...
                <>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex shrink-0 gap-3">
                      <button onClick={() => abrirEdicao(c)} className="text-xs text-slate-400 hover:text-slate-200">
                        Editar
                      </button>
                      <button
