...
export function ForgotPassword({ onBack }: { onBack: () => void }) {
  ...
  const start = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!r.ok) {
        const message = (data?.error ?? 'Nao foi possivel enviar o codigo.') as string;
        if (r.status === 429 || /muitas tentativas|limite|tente novamente em/i.test(message)) {
          setError('Muitas tentativas de recuperação. Tente novamente em 1 hora.');
          setRateLimited(true);
          return;
        }
  ...
  const verify = async (e: React.FormEvent) => {
    ...
    try {
      const r = await fetch('/api/auth/password/verify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        // CORRIGIDO: o backend espera a chave "contact" (e-mail), não "phone" —
        // era por isso que aparecia "E-mail e código são obrigatórios" mesmo
        // com o código certo digitado.
        body: JSON.stringify({ contact: contact || phone, code: c }),
      });
      const data = await r.json();
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="text-center flex-1">
          ...
          <p className="mt-2 text-sm text-slate-400">
            {step === 'contact' && 'Informe seu e-mail cadastrado.'}
            {step === 'code' && 'Digite o código enviado para o seu e-mail.'}
      ...
      {rateLimited ? (
        <div className="space-y-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-center">
          <p className="text-sm font-medium text-amber-300">Muitas tentativas de recuperação.</p>
          <p className="text-xs text-amber-200/80">Você pode tentar novamente em 1 hora.</p>
        </div>
      ) : error ? (
      ...
      {!rateLimited && step === 'contact' && (
        <form onSubmit={start} className="space-y-3">
          <input className={input} placeholder="E-mail cadastrado" value={contact}
            onChange={(e) => setContact(e.target.value)} required type="email" />
          <button type="submit" className={secondaryBtn} disabled={busy || !contact}>
            {busy ? 'Enviando código...' : 'Enviar código por e-mail'}
          </button>
        </form>
      )}
      ...
      {step === 'code' && (
        <form onSubmit={verify} className="space-y-3">
          <div className="rounded-xl border border-primary/20 bg-primary/10 p-4 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">Digite o código recebido em {contact || phone}.</p>
          </div>
