...
export function InstallPrompt() {
  ...
  useEffect(() => {
    ...
    // Não mostrar durante o onboarding
    if (window.localStorage.getItem('miar-onboarding-pending') === '1') return;
    setVisible(true);
  ...
  return (
    <aside
      role="status"
      aria-label="Instalar o aplicativo MIAR Gestor Estabelecimento"
      style={{
        position: 'fixed',
        zIndex: 9999,
        right: 20,
    ...
    >
      <div style={{ position: 'relative', width: '100%', maxWidth: 440, padding: 28, border: '1px solid rgba(0,230,242,0.35)', borderRadius: 18, background: '#0B1A10', color: '#F2F7F3', boxShadow: '0 12px 40px rgba(0,0,0,0.5)' }}>
        <img src={appIcon} alt="MIAR AI/FOOD" width={56} height={56} style={{ borderRadius: 14 }} />
        <div style={{ marginTop: 18, fontWeight: 700, fontSize: 20, lineHeight: 1.2, color: '#9EF01A' }}>
        ...
        >
          {deferred ? 'Instalar aplicativo' : 'Instale MIAR AI/FOOD'}
        </button>
          <button onClick={dismiss} style={{ border: '1px solid rgba(245,238,230,0.25)', borderRadius: 10, padding: '10px 18px', fontWeight: 700, fontSize: 14, background: 'transparent', color: '#F2F7F3', cursor: 'pointer' }}>
