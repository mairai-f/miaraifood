...
export default function EstoquePage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetch('/api/stock', { headers: { Authorization: `Bearer ${getToken()}`, ...lojaHeaders() } })
  ...
  const confirmRemove = async () => {
    ...
    setDeleting(true);
    try {
      await fetch(`/api/stock/${deleteTarget.id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${getToken()}`, ...lojaHeaders() } });
      await load();
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-xl font-semibold flex items-center gap-2"><Package className="h-5 w-5 text-emerald-400" /> {t('estoque.titulo')}</h1>
            </div>
          </div>
      ...
      <ConfirmModal
        ...
        actionName={`Excluir "${deleteTarget?.name}" do Estoque`}
        description="Esta ação removerá permanentemente o item e seu histórico de movimentação do saldo de estoque."
        confirmText="Continuar e Excluir Item"
