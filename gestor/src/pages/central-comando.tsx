...
// O Gestor roda sob um prefixo de caminho na Vercel (BASE_PATH=/gestor/,
// ver artifacts/gestor/package.json) — um src de iframe com caminho absoluto
// "/app/mesas" escapa desse prefixo e cai na raiz do domínio, dando 404.
// import.meta.env.BASE_URL já reflete o BASE_PATH usado no build (com "/"
// no final), então prefixamos aqui uma vez em vez de em cada uso.
const BASE = import.meta.env.BASE_URL.replace(/\/$/, "");

const MODULOS: Modulo[] = [
  { url: `${BASE}/app/mesas`, nome: 'Mesas & Comandas', interno: true },
  { url: `${BASE}/app/cozinha`, nome: 'Cozinha KDS', interno: true },
  { url: `${BASE}/app/pdv`, nome: 'PDV Caixa Operacional', interno: true },
  { url: `${BASE}/catalogo`, nome: 'Cardápio Digital', interno: true },
  { url: `${BASE}/minha-ia`, nome: 'IA Ária', interno: true },
  { url: `${BASE}/central-de-lojas`, nome: 'Central de Lojas', interno: true },
  { url: `${BASE}/marketing`, nome: 'Marketing IA', interno: true },
  { url: `${BASE}/estoque/auditoria`, nome: 'Auditoria de estoque', interno: true },
  { url: `${BASE}/estoque/codigo-barras`, nome: 'Código de barras', interno: true },
  { url: `${BASE}/socios`, nome: 'Sócios', interno: true },
  { url: `${BASE}/camera-local`, nome: 'Câmera', interno: true },
  { url: `${BASE}/drive-thru`, nome: 'Drive-thru', interno: true },
  { url: `${BASE}/feed`, nome: 'Feed interno', interno: true },
  { url: `${BASE}/mural-empregos`, nome: 'Mural de empregos', interno: true },
  { url: `${BASE}/publicar-feed`, nome: 'Publicar no Feed', interno: true },
  { url: `${BASE}/motor-demanda`, nome: 'Oportunidades da cidade', interno: true },
];

const PADRAO = [0, 1, 2, 3]; // Mesas & Comandas, Cozinha KDS, PDV Caixa, Cardápio Digital
...
export default function CentralComando() {
  ...
  const [monitoresDetectados, setMonitoresDetectados] = useState<any[]>([]);
    ...
    if ('getScreenDetails' in window) {
      try {
        ...
        if (details?.screens && details.screens.length > 0) {
          setMonitoresDetectados(details.screens);
        }
      } catch (e) {
        // Fallback standard dual monitor support
      }
    }
  };
  ...
  const desvincular = (i: number, targetScreen?: any) => {
    const modulo = MODULOS[sel[i]];
    if (targetScreen) {
      window.open(
        modulo.url,
        `miar-${modulo.nome}`,
        `left=${targetScreen.left},top=${targetScreen.top},width=${targetScreen.width},height=${targetScreen.height},resizable=yes,scrollbars=yes`
      );
    } else {
      window.open(
        modulo.url,
  ...
  const btn = (active: boolean): React.CSSProperties => ({
    ...
    fontWeight: 700,
    fontFamily: 'inherit',
    background: active ? 'var(--miar-ciano, #9EF01A)' : 'var(--miar-surface, #0B1A10)',
    color: active ? '#06100A' : 'var(--miar-muted, #8FA396)',
    border: active ? 'none' : '1px solid var(--miar-line, #16301F)',
  });
  ...
  return (
    <div
      style={{
        ...
        background: 'var(--miar-base, #06100A)', color: 'var(--miar-text, #F2F7F3)',
    ...
    >
      ...
      <div
        style={{
          ...
          borderBottom: '1px solid var(--miar-line, #16301F)', flexWrap: 'wrap',
        }}
      >
        ...
        <span style={{ fontSize: 12, color: 'var(--miar-muted, #8FA396)' }}>
          ...
          <div style={{ display: 'flex', gap: 8, height: 96 }}>
            {[0, 1, 2, 3].filter((i) => i !== maximizado).map((i) => (
              <button
                ...
                style={{
                  flex: 1, borderRadius: 10, border: '1px solid var(--miar-line, #16301F)',
                  background: 'var(--miar-surface, #0B1A10)', color: 'var(--miar-muted, #8FA396)',
  ...
  return (
    <div
      ...
      style={{
        ...
        border: '1px solid var(--miar-line, #16301F)', borderRadius: 14,
        overflow: 'hidden', background: 'var(--miar-surface, #0B1A10)',
      }}
    >
      <div
        style={{
      ...
      >
        <select
          ...
          style={{
            flex: 1, minWidth: 100, background: 'var(--miar-base, #06100A)',
            color: 'var(--miar-text, #F2F7F3)', border: '1px solid var(--miar-line, #16301F)',
      ...
      <iframe
        ...
        style={{ flex: 1, width: '100%', border: 'none', background: 'var(--miar-base, #06100A)' }}
      />
    </div>
  );
}
