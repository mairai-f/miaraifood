...
export default function ConviteEntregador() {
  ...
  // Migração pra domínio próprio (05/09/2026): o backend sempre manda
  // accessPath como "/entregador/?token=..." (contrato pensado pro pipeline
  // compartilhado, path-based). Com VITE_ENTREGADOR_URL setado (Entregador
  // já em domínio próprio), esse app já FICA na raiz — tira o prefixo
  // "/entregador" repetido antes de colar na URL base.
  const entregadorUrl = import.meta.env.VITE_ENTREGADOR_URL as string | undefined;
  const urlCompleta = (path: string) =>
    entregadorUrl
      ? `${entregadorUrl}${path.replace(/^\/entregador/, '')}`
