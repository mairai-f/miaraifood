...
import { ShieldCheck, MapPin, Wifi, Save, Info, Languages, QrCode } from 'lucide-react';
...
export default function Seguranca() {
  ...
  const [salvandoPixKey, setSalvandoPixKey] = useState(false);
  const [pixKeySalva, setPixKeySalva] = useState(false);
  ...
  useEffect(() => {
    fetch('/api/settings', { headers: { Authorization: `Bearer ${getToken()}` } })
      ...
      .then((s) => {
        ...
        setAllowPayAtTable(Boolean(s.allowPayAtTable));
        setPixKey(typeof s.pixKey === 'string' ? s.pixKey : '');
      })
      .finally(() => setCarregando(false));
  }, []);

  const alternarPagamentoNaMesa = async () => {
    const novoValor = !allowPayAtTable;
    setAllowPayAtTable(novoValor);
    setSalvandoPagamento(true);
    try {
      await fetch('/api/settings', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${getToken()}` },
        body: JSON.stringify({ allowPayAtTable: novoValor }),
      });
    } catch {
      setAllowPayAtTable(!novoValor);
    } finally {
  ...
  const salvarPixKey = async () => {
    ...
    try {
      ...
      setPixKeySalva(true);
      setTimeout(() => setPixKeySalva(false), 2500);
    } finally {
      setSalvandoPixKey(false);
    }
  };
  ...
  return (
    <div className="mx-auto max-w-5xl p-4 sm:p-6">
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
        <p className="mb-2 flex items-center gap-2 font-semibold text-slate-100">
          <QrCode className="h-4 w-4 text-emerald-400" /> QR Menu — pagamento na mesa
        </p>
        <p className="mb-4 text-sm text-slate-400">
          Permite que o cliente feche a conta e escolha "pagar na mesa" pelo app da mesa (escaneando o QR),
          gerando um recibo/cupom. Nenhuma cobrança real é feita — é só o fluxo de fechamento e recibo.
        </p>
        <div className="flex items-center justify-between">
          <div>
            <p className="font-semibold text-slate-100">Permitir pagamento na mesa</p>
            <p className="text-sm text-slate-400">Desligado por padrão.</p>
          </div>
          <button
            onClick={alternarPagamentoNaMesa}
            disabled={salvandoPagamento}
            className={`h-7 w-12 rounded-full transition disabled:opacity-60 ${allowPayAtTable ? 'bg-emerald-500' : 'bg-slate-700'}`}
          >
            <span className={`block h-5 w-5 translate-x-1 rounded-full bg-white transition ${allowPayAtTable ? 'translate-x-6' : ''}`} />
          </button>
        </div>

        {allowPayAtTable && (
          <div className="mt-4 border-t border-slate-800 pt-4">
            <label className="mb-2 block font-semibold text-slate-100" htmlFor="pixKeyInput">
              Chave Pix pra receber o pagamento na mesa
            </label>
            <p className="mb-2 text-sm text-slate-400">
              CPF/CNPJ, e-mail, telefone ou chave aleatória — é essa chave que aparece pro cliente no
              recibo da mesa.
            </p>
            <div className="flex flex-col gap-2 sm:flex-row">
              <input
                ...
                placeholder="Ex: 00020126...ou email@seurestaurante.com.br"
              ...
              >
                {salvandoPixKey ? 'Salvando...' : pixKeySalva ? 'Salvo ✓' : 'Salvar chave Pix'}
              </button>
            </div>
          </div>
        )}
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-slate-400">
          Trava o uso do caixa, atendente, cozinha e entregador a "dentro do restaurante".
