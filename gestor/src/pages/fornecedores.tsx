...
export default function FornecedoresPage() {
  ...
  const confirmRemove = async () => {
    ...
    setDeleting(true);
    try {
      await fetch(`/api/fornecedores/${deleteTarget.id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${getToken()}` } });
      await load();
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-4xl px-4 py-6">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs uppercase tracking-widest text-sky-400">{t('fornecedores.subtitulo')}</p>
              <h1 className="text-xl font-semibold flex items-center gap-2"><Truck className="h-5 w-5 text-sky-400" /> {t('fornecedores.titulo')}</h1>
            </div>
          </div>
          <button onClick={() => { resetForm(); setShowForm(true); }}
            ...
            <Plus className="h-4 w-4" /> {t('fornecedores.novo')}
          </button>
        </div>

        {/* Busca */}
        <input className={`${inp} mb-4`} placeholder={t('fornecedores.buscar_placeholder')}
      ...
      <ConfirmModal
        ...
        actionName={`Excluir Fornecedor "${deleteTarget?.name}"`}
        description="Esta ação removerá o fornecedor da lista de distribuidores do seu estabelecimento."
        confirmText="Continuar e Excluir Fornecedor"
