...
export type AppTab = 'home' | 'search' | 'pedidos' | 'feed' | 'jobs' | 'chat' | 'profile' | 'trends' | 'artista';
...
export interface MenuItemAdicional { id: string; label: string; priceCents?: number }
export interface MenuItem {
  id: string; name: string; price: number; category?: string; description?: string;
  adicionais?: MenuItemAdicional[];
  /** Nomes dos ingredientes da ficha técnica — vira checkbox "tirar X" (mesma origem do QR Menu). */
  ingredientes?: string[];
}
