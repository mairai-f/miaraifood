...
import { User, Star, Clock, Repeat, Leaf, Settings, ChevronRight, Camera, Send, Bot, Loader2, LogOut, X, MapPin, PawPrint } from 'lucide-react';
...
type Section = 'main' | 'history' | 'loyalty' | 'nutrition' | 'prefs' | 'addresses' | 'pets';

type Pet = { id: string; name: string; species: string; breed: string | null; weightKg: number | null; allergies: string | null; vaccines: string | null; notes: string | null };
type PetAppointment = { id: string; serviceName: string; startsAt: string; durationMinutes: number; status: string; petId: string };
type PetService = { id: string; name: string; durationMinutes: number; priceCents: number };

function PetshopSection() {
  const token = getClientToken();
  const [pets, setPets] = useState<Pet[]>([]);
  const [appointments, setAppointments] = useState<PetAppointment[]>([]);
  const [form, setForm] = useState({ name: '', species: 'Cão', breed: '', weightKg: '', allergies: '', vaccines: '' });
  const [appointment, setAppointment] = useState({ restaurantId: '', petId: '', serviceName: 'Banho e tosa', startsAt: '' });
  ...
  const load = async () => {
    ...
    const [petsResponse, appointmentsResponse] = await Promise.all([
      fetch('/api/client/pets', { headers }),
      fetch('/api/client/appointments', { headers }),
    ]);
    if (petsResponse.ok) setPets(await petsResponse.json() as Pet[]);
    if (appointmentsResponse.ok) setAppointments(await appointmentsResponse.json() as PetAppointment[]);
  };

  useEffect(() => {
    void load();
    fetch('/api/restaurants').then((response) => response.ok ? response.json() : []).then(setRestaurants).catch(() => {});
  }, []);

  useEffect(() => {
    if (!appointment.restaurantId) { setServices([]); return; }
    fetch(`/api/client/petshop-services?restaurantId=${encodeURIComponent(appointment.restaurantId)}`)
      .then((response) => response.ok ? response.json() : []).then(setServices).catch(() => setServices([]));
  }, [appointment.restaurantId]);

  const createPet = async (event: React.FormEvent) => {
    event.preventDefault();
    const response = await fetch('/api/client/pets', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ ...form, weightKg: form.weightKg ? Number(form.weightKg) : null }) });
    setMessage(response.ok ? 'Pet cadastrado.' : ((await response.json().catch(() => ({}))).error ?? 'Não foi possível cadastrar o pet.'));
    if (response.ok) { setForm({ name: '', species: 'Cão', breed: '', weightKg: '', allergies: '', vaccines: '' }); await load(); }
  };

  const createAppointment = async (event: React.FormEvent) => {
    event.preventDefault();
    const selectedService = services.find((service) => service.id === appointment.serviceName);
    if (!selectedService) { setMessage('Escolha um serviço cadastrado.'); return; }
    const response = await fetch('/api/client/appointments', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ ...appointment, serviceName: selectedService.name, startsAt: new Date(appointment.startsAt).toISOString(), durationMinutes: selectedService.durationMinutes }) });
    setMessage(response.ok ? 'Atendimento agendado.' : ((await response.json().catch(() => ({}))).error ?? 'Não foi possível agendar.'));
    if (response.ok) { setAppointment({ ...appointment, petId: '', startsAt: '' }); await load(); }
  };

  if (!token) return <p className="px-4 pt-6 text-sm text-slate-500">Entre na conta para cadastrar pets.</p>;
  return <div className="space-y-5 px-4 pt-5">
    ...
    <form onSubmit={createPet} className="rounded-2xl border border-slate-800 bg-slate-900 p-4 space-y-3">
      <h2 className="font-semibold">Novo pet</h2>
      <input required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Nome do animal" className="w-full rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm" />
      <div className="grid grid-cols-2 gap-2"><input required value={form.species} onChange={e => setForm({ ...form, species: e.target.value })} placeholder="Espécie" className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm" /><input value={form.breed} onChange={e => setForm({ ...form, breed: e.target.value })} placeholder="Raça" className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm" /></div>
      <div className="grid grid-cols-2 gap-2"><input type="number" min="0" step="0.01" value={form.weightKg} onChange={e => setForm({ ...form, weightKg: e.target.value })} placeholder="Peso (kg)" className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm" /><input value={form.allergies} onChange={e => setForm({ ...form, allergies: e.target.value })} placeholder="Alergias" className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm" /></div>
      <input value={form.vaccines} onChange={e => setForm({ ...form, vaccines: e.target.value })} placeholder="Vacinas" className="w-full rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm" />
      <button type="submit" className="w-full rounded-xl bg-emerald-500 py-3 text-sm font-semibold text-slate-950">Cadastrar pet</button>
    </form>
    {pets.length > 0 && <div className="space-y-2">{pets.map(pet => <div key={pet.id} className="rounded-xl border border-slate-800 bg-slate-900 p-3"><p className="font-semibold">{pet.name} <span className="text-xs font-normal text-slate-400">{pet.species}{pet.breed ? ` · ${pet.breed}` : ''}</span></p><p className="mt-1 text-xs text-slate-500">{pet.weightKg ? `${pet.weightKg} kg` : 'Peso não informado'}{pet.allergies ? ` · Alergias: ${pet.allergies}` : ''}</p></div>)}</div>}
    <form onSubmit={createAppointment} className="rounded-2xl border border-slate-800 bg-slate-900 p-4 space-y-3">
      <h2 className="font-semibold">Agendar atendimento</h2>
      <select required value={appointment.restaurantId} onChange={e => setAppointment({ ...appointment, restaurantId: e.target.value })} className="w-full rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm"><option value="">Escolha o estabelecimento</option>{restaurants.map(restaurant => <option key={restaurant.id} value={restaurant.id}>{restaurant.name}</option>)}</select>
      <select required value={appointment.petId} onChange={e => setAppointment({ ...appointment, petId: e.target.value })} className="w-full rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm"><option value="">Escolha o pet</option>{pets.map(pet => <option key={pet.id} value={pet.id}>{pet.name}</option>)}</select>
      <select required value={appointment.serviceName} onChange={e => setAppointment({ ...appointment, serviceName: e.target.value })} className="w-full rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm"><option value="">Escolha o serviço</option>{services.map(service => <option key={service.id} value={service.id}>{service.name} · {service.durationMinutes} min · R$ {(service.priceCents / 100).toFixed(2).replace('.', ',')}</option>)}</select><input required type="datetime-local" value={appointment.startsAt} onChange={e => setAppointment({ ...appointment, startsAt: e.target.value })} className="w-full rounded-xl border border-slate-700 bg-slate-950 p-3 text-sm" />
      <button type="submit" className="w-full rounded-xl bg-sky-400 py-3 text-sm font-semibold text-slate-950">Agendar</button>
    </form>
    {appointments.length > 0 && <div className="space-y-2"><h2 className="font-semibold">Meus atendimentos</h2>{appointments.map(item => <div key={item.id} className="rounded-xl border border-slate-800 bg-slate-900 p-3 text-sm"><p className="font-medium">{item.serviceName}</p><p className="mt-1 text-xs text-slate-400">{new Date(item.startsAt).toLocaleString('pt-BR')} · {item.durationMinutes} min · {item.status}</p></div>)}</div>}
  </div>;
}
...
export default function ProfileTab({
  ...
  return (
    <div className="client-dark-theme min-h-screen bg-slate-950 text-slate-100 pb-24">
      {/* Section header */}
      {section !== 'main' && (
          ...
          <motion.div key="main" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="px-4 pt-6">
            ...
            <div className="space-y-2">
              {[
                ...
                { section: 'pets' as const, icon: '🐾', label: 'Meus pets e atendimentos', sub: 'Ficha e agenda Petshop' },
