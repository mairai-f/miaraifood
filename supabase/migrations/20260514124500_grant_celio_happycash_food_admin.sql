...
VALUES
  ('food', 'pricing.manage', true),
  ('food', 'thermal.printer', true),
  ('food', 'bematech.print', true),
  ('food', 'thermal.printer.annual', true),
  ('food', 'card.terminal.annual', true),
  ('food_offline', 'pricing.manage', true),
  ('food_offline', 'thermal.printer', true),
  ('food_offline', 'bematech.print', true),
  ('food_offline', 'thermal.printer.annual', true),
  ('food_offline', 'card.terminal.annual', true)
...
BEGIN
  ...
  IF current_subscription_id IS NULL THEN
    ...
    VALUES (
      ...
      target_user_id::text,
      jsonb_build_object(
        'source', 'happycash_food_admin_test_access',
  ...
  ELSE
    ...
    SET
      ...
      metadata = COALESCE(metadata, '{}'::jsonb) || jsonb_build_object(
        'replaced_at', now(),
        'reason', 'happycash_food_admin_test_access'
      ),
      updated_at = now()
