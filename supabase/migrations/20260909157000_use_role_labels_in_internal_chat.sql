-- Email is an authentication credential, never a chat display name.
...
BEGIN
  ...
  WITH visible_conversations AS (
    SELECT conversation.*,
      CASE WHEN conversation.kind = 'direct' THEN CASE WHEN conversation.created_by_user_id = auth.uid()
        THEN COALESCE(conversation.direct_recipient_user_id, (SELECT member.user_id FROM public.internal_chat_members AS member WHERE member.conversation_id = conversation.id AND member.user_id <> auth.uid() ORDER BY member.joined_at LIMIT 1))
        ELSE conversation.created_by_user_id END END AS resolved_peer_user_id
    FROM public.internal_chat_conversations AS conversation
    WHERE conversation.store_account_id = v_store_id
      AND (v_is_admin OR EXISTS (SELECT 1 FROM public.internal_chat_members AS member WHERE member.conversation_id = conversation.id AND member.user_id = auth.uid()))
  )
