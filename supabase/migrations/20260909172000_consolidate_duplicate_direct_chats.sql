-- Existing historical duplicates are intentionally not merged here: audit
-- records must never be reassigned implicitly. The UI displays one canonical
-- channel and the function below prevents any new duplicate from existing.
-- Ensure both participants are members whether an administrator or a
-- collaborator created the private channel.
...
BEGIN
  ...
  WHERE conversation.store_account_id = v_store_id AND conversation.kind = 'direct'
