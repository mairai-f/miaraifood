...
export const getDesktopFiscalBlockedMessage = ({
  ...
  if (!isDesktop) {
    return 'No web, o MIAR AI/FOOD trabalha somente com cupom/recibo nao fiscal. A NFC-e fica disponivel apenas no MIAR AI/FOOD Desktop PRO.';
  }

  if (!licensed || !activation) {
    return 'Ative e valide esta maquina no MIAR AI/FOOD Desktop PRO para liberar o modulo fiscal NFC-e.';
  }

  if (activation.appContext !== 'happycash') {
    return 'Este modulo fiscal pertence ao MIAR AI/FOOD Desktop PRO.';
  }
