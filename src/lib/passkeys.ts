...
export const getPasskeySupportErrorMessage = () => {
  ...
  if (isLoopbackHost(window.location.hostname)) {
    return 'Biometria por passkey ficara disponivel nos dominios oficiais do HappyCash. No localhost, continue usando senha.';
  }

  if (!window.location.hostname.endsWith(PASSKEY_RP_ID)) {
    return 'Este dominio ainda nao faz parte da biometria configurada do HappyCash.';
  }
