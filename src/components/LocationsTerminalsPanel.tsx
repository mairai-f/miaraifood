...
export function LocationsTerminalsPanel() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (accountError || !accountId) throw accountError ?? new Error('Empresa MIAR AI/FOOD nao encontrada.');
  ...
  const handleSaveTerminal = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!currentTerminal) {
      toast.info('O terminal aparece automaticamente depois da ativacao do MIAR AI/FOOD Desktop.');
      return;
    }
  ...
  const deleteAuthorizationDescription = pendingDeleteAction?.type === 'terminal'
    ? 'O terminal sera removido do banco. Se ele estiver vinculado a um MIAR AI/FOOD Desktop, aquela maquina sera bloqueada e voltara para a tela inicial da license key.'
    : pendingDeleteAction?.type === 'location'
                  ...
                  <DialogContent>
                    <form onSubmit={handleSaveTerminal}>
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          O cadastro nasce na ativacao do MIAR AI/FOOD Desktop. Aqui voce ajusta nome, filial e bloqueio do terminal.
                        </DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4 py-4">
                        <div className="space-y-1">
