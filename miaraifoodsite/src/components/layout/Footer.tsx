...
function SocialCard({ social }: { social: SocialLink }) {
    const Icon = socialIcons[social.icon] || FaWhatsapp;
...
export function Footer() {
    ...
    const isBlog = pathname?.includes('/conteudos');
    const isBlogDetail = pathname?.includes('/conteudos/') && pathname.split('/conteudos/')[1]?.length > 0;
    const isGallery = pathname?.includes('/gallery');

    const previewSocials = portfolioData.personal.socialLinks.slice(0, 4);
                        ...
                        >
                            ...
                            <div className="flex-1 flex flex-col px-[8vw] pt-[4vh] pb-0 justify-between relative">
                                <div className="flex-1 flex flex-col justify-center max-w-[1600px] w-full mx-auto relative">
                                    ...
                                    <div className="w-full grid grid-cols-1 md:grid-cols-4 gap-x-[5vw] gap-y-[4vh]">
                                        <FooterColumn title="Produto">
                                            <FooterLink href="/funcionalidades" onClick={closeExpanded}>Funcionalidades</FooterLink>
                                            <FooterLink href="/recursos" onClick={closeExpanded}>Recursos Integrados</FooterLink>
                                            <FooterLink href="/central-de-ajuda" onClick={closeExpanded}>Central de Ajuda</FooterLink>
                                            <FooterLink href="/telas" onClick={closeExpanded}>Telas do Sistema</FooterLink>
                                        </FooterColumn>

                                        <FooterColumn title="Empresa">
                                            <FooterLink href="/sobre" onClick={closeExpanded}>Sobre o MIAR AI/FOOD</FooterLink>
                                            <FooterLink href="/conteudos" onClick={closeExpanded}>Conteúdos</FooterLink>
                                            <FooterLink href="/planos" onClick={closeExpanded}>Planos</FooterLink>
                                        </FooterColumn>

                                        <FooterColumn title="Legal">
                                            <FooterLink href="/termos" onClick={closeExpanded}>Termos de Uso</FooterLink>
                                            <FooterLink href="/termos" onClick={closeExpanded}>Política de Privacidade</FooterLink>
                                            <FooterLink href="/termos" onClick={closeExpanded}>LGPD</FooterLink>
                                        </FooterColumn>

                                        <FooterColumn title="Contato">
                                            <FooterLink href="https://wa.me/5512988918792?text=Ol%C3%A1%2C%20gostaria%20de%20saber%20mais%20sobre%20o%20MIAR AI/FOOD!" target="_blank" onClick={closeExpanded}>WhatsApp</FooterLink>
                                            <FooterLink href={`mailto:${portfolioData.personal.email}`} onClick={closeExpanded}>Email</FooterLink>
                                        </FooterColumn>
                                    </div>
                                </div>

                                {/* Bottom Brand Name - Scaled and slightly lifted */}
                                <div className="mt-12 pt-6 pb-2 w-full flex justify-center items-center border-t border-zinc-200 dark:border-white/10">
                                    <motion.h2
                                        initial={{ opacity: 0, y: 10 }}
                                        animate={isAboutExpanded ? { opacity: 0, y: 20 } : { opacity: 1, y: -10 }}
                                        transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
                                        className="text-[clamp(3rem,15vw,20rem)] font-black leading-none text-zinc-900 dark:text-white tracking-tighter select-none text-center pointer-events-none"
                                    >
                                        MIAR AI/FOOD
                                    </motion.h2>
                                </div>
...
function FooterLink({ href, children, target, onClick }: { href: string; children: React.ReactNode; target?: string, onClick?: () => void }) {
    return (
        <motion.div
            whileHover={{ x: 5 }}
...
function AboutHoverMenu({ tNav, onExpandChange }: { tNav: (key: string) => string; onExpandChange: (expanded: boolean) => void }) {
    ...
    const subLinks = [
    { href: '/central-de-ajuda', label: tNav('menu.achievements') },
    { href: '/recursos', label: tNav('menu.skills') },
    { href: '/sobre', label: tNav('menu.experience') },
    { href: '/funcionalidades', label: tNav('menu.projects') },
    { href: '/conteudos', label: tNav('menu.blog') },
    { href: '/telas', label: tNav('menu.gallery') },
];
