...
export const HardSkills = () => {
  ...
  const categorizedSkills = useMemo(() => {
    ...
    portfolioData.hardSkills.forEach(skill => {
      ...
      if (['pagamentos'].includes(cat)) {
        groups['pagamentos'].push(skill);
      } else if (['fiscal'].includes(cat)) {
        groups['fiscal'].push(skill);
      } else if (['operacao'].includes(cat)) {
        groups['operacao'].push(skill);
      } else {
        groups['conectividade'].push(skill);
      }
    });
    return groups;
  }, []);
    ...
    <section id="hard-skills" className="w-full bg-background pt-32 md:pt-40 lg:pt-52 pb-24 relative overflow-hidden">
      ...
      <div className="w-full max-w-[1400px] px-4 md:px-8 mx-auto relative z-10 flex flex-col items-center">
        ...
        <div className="text-center mb-10 md:mb-16">
          ...
          >
            Tudo o que o MIAR AI/FOOD precisa para conectar a operação da sua empresa em um só lugar.
          </motion.span>
        </div>
        ...
        <motion.div 
            ...
            className="hidden lg:flex w-full items-center bg-white dark:bg-[#111111] border border-black/5 dark:border-white/10 rounded-full mb-8 shadow-sm relative z-20 overflow-hidden"
        >
        ...
        {/* 4 Columns Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 w-full relative z-10 items-stretch">
          ...
          >
            <div className="shrink-0 lg:hidden inline-block px-6 py-2.5 bg-foreground text-background rounded-full font-bold text-sm mb-4 text-center w-max mx-auto shadow-md">
              Pagamentos
            </div>
            <div
              data-lenis-prevent={expanded.pagamentos ? "true" : undefined}
              onWheel={expanded.pagamentos ? (e) => e.stopPropagation() : undefined}
              className={cn(
                ...
                expanded.pagamentos ? "overflow-y-auto" : "overflow-hidden"
              )}
            >
              <AnimatePresence>
                {categorizedSkills['pagamentos'].slice(0, expanded.pagamentos ? undefined : 3).map((skill, idx) => (
                  <SkillCard key={skill.name} skill={skill} delay={idx * 0.05} />
                ))}
              </AnimatePresence>
            </div>
            {categorizedSkills['pagamentos'].length > 3 && (
              <div className="shrink-0 mt-auto pt-4">
                <button
                  onClick={() => toggleExpand('pagamentos')}
                ...
                >
                  {expanded.pagamentos ? (
                    ...
                    <>Ver Mais ({categorizedSkills['pagamentos'].length - 3}) <ChevronDown className="w-4 h-4" /></>
                  )}
                </button>
              </div>
            )}
          ...
          >
            ...
            <div
              data-lenis-prevent={expanded.fiscal ? "true" : undefined}
              onWheel={expanded.fiscal ? (e) => e.stopPropagation() : undefined}
              className={cn(
                "flex flex-col gap-4 flex-grow min-h-0 transition-all duration-300 pr-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-black/10 dark:[&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full",
                expanded.fiscal ? "overflow-y-auto" : "overflow-hidden"
              )}
            >
              <AnimatePresence>
                {categorizedSkills['fiscal'].slice(0, expanded.fiscal ? undefined : 3).map((skill, idx) => (
                  <SkillCard key={skill.name} skill={skill} delay={idx * 0.05} />
                ))}
              </AnimatePresence>
            </div>
            {categorizedSkills['fiscal'].length > 3 && (
              <div className="shrink-0 mt-auto pt-4">
                <button
                  onClick={() => toggleExpand('fiscal')}
                  className="w-full py-3 flex items-center justify-center gap-2 rounded-xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-neutral-600 dark:text-neutral-400 hover:text-foreground font-medium text-sm transition-colors border border-black/5 dark:border-white/5"
                >
                  {expanded.fiscal ? (
                    <>Ver menos <ChevronUp className="w-4 h-4" /></>
                  ) : (
                    <>Ver mais ({categorizedSkills['fiscal'].length - 3}) <ChevronDown className="w-4 h-4" /></>
                  )}
                </button>
              </div>
            )}
          ...
          >
            ...
            <div
              data-lenis-prevent={expanded.operacao ? "true" : undefined}
              onWheel={expanded.operacao ? (e) => e.stopPropagation() : undefined}
              className={cn(
                "flex flex-col gap-4 flex-grow min-h-0 transition-all duration-300 pr-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-black/10 dark:[&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full",
                expanded.operacao ? "overflow-y-auto" : "overflow-hidden"
              )}
            >
              <AnimatePresence>
                {categorizedSkills['operacao'].slice(0, expanded.operacao ? undefined : 3).map((skill, idx) => (
                  <SkillCard key={skill.name} skill={skill} delay={idx * 0.05} />
                ))}
              </AnimatePresence>
            </div>
            {categorizedSkills['operacao'].length > 3 && (
              <div className="shrink-0 mt-auto pt-4">
                <button
                  onClick={() => toggleExpand('operacao')}
                  className="w-full py-3 flex items-center justify-center gap-2 rounded-xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-neutral-600 dark:text-neutral-400 hover:text-foreground font-medium text-sm transition-colors border border-black/5 dark:border-white/5"
                >
                  {expanded.operacao ? (
                    <>Ver menos <ChevronUp className="w-4 h-4" /></>
                  ) : (
                    <>Ver Mais ({categorizedSkills['operacao'].length - 3}) <ChevronDown className="w-4 h-4" /></>
                  )}
                </button>
              </div>
            )}
          ...
          >
            ...
            <div
              data-lenis-prevent={expanded.conectividade ? "true" : undefined}
              onWheel={expanded.conectividade ? (e) => e.stopPropagation() : undefined}
              className={cn(
                "flex flex-col gap-4 flex-grow min-h-0 transition-all duration-300 pr-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-black/10 dark:[&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full",
                expanded.conectividade ? "overflow-y-auto" : "overflow-hidden"
              )}
            >
              <AnimatePresence>
                {categorizedSkills['conectividade'].slice(0, expanded.conectividade ? undefined : 3).map((skill, idx) => (
                  <SkillCard key={skill.name} skill={skill} delay={idx * 0.05} />
                ))}
              </AnimatePresence>
            </div>
            {categorizedSkills['conectividade'].length > 3 && (
              <div className="shrink-0 mt-auto pt-4">
                <button
                  onClick={() => toggleExpand('conectividade')}
                  className="w-full py-3 flex items-center justify-center gap-2 rounded-xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-neutral-600 dark:text-neutral-400 hover:text-foreground font-medium text-sm transition-colors border border-black/5 dark:border-white/5"
                >
                  {expanded.conectividade ? (
                    <>Ver menos <ChevronUp className="w-4 h-4" /></>
                  ) : (
                    <>Ver Mais ({categorizedSkills['conectividade'].length - 3}) <ChevronDown className="w-4 h-4" /></>
                  )}
                </button>
              </div>
            )}
      ...
      <div className="mb-4 relative z-10">
        <div className="flex flex-wrap justify-between items-start mb-4 gap-2">
          ...
          <span className={cn(
            ...
            {skill.level === 'expert' ? 'Premium' : skill.level === 'advanced' ? 'Avançado' : skill.level === 'intermediate' ? 'Essencial' : 'Básico'}
          </span>
          )}
        </div>
