...
export default function StatsSection({ scrollYProgress, showOnly }: { scrollYProgress?: any, showOnly?: 'top' | 'bottom' }) {
    ...
    const galleryItems = [
        {
            images: [
                { src: '/telasdosistema/pdvfrentedecaixa.png', alt: 'Frente de Caixa' },
                { src: '/telasdosistema/telademesas.png', alt: 'Tela de Mesas' },
                { src: '/telasdosistema/cardaioqrcode.png', alt: 'Cardápio QR Code' }
            ]
        },
        {
            images: [
                { src: '/telasdosistema/gestaodofiado.png', alt: 'Gestão do Fiado' },
                { src: '/telasdosistema/telakdscozinha.png', alt: 'Tela KDS Cozinha' },
                { src: '/telasdosistema/paineldecontrole.png', alt: 'Painel de Controle' }
            ]
        },
        {
            images: [
                { src: '/telasdosistema/controleseuestoque.png', alt: 'Controle de Estoque' },
                { src: '/telasdosistema/cadastreprodutos.png', alt: 'Cadastro de Produtos' },
                { src: '/telasdosistema/configuraçoes.png', alt: 'Configurações' }
            ]
        }
    ];

    useEffect(() => {
        const flatImages = galleryItems.flatMap(item => item.images);
        const shuffledImages = [...flatImages].sort(() => 0.5 - Math.random());
            ...
            {(showOnly === 'top' || !showOnly) && (
                <>
                    <div className="max-w-6xl mx-auto px-6 w-full pt-32 pb-16 text-center space-y-4">
                        ...
                        >
                            Veja a plataforma em ação através de telas do sistema e conteúdos que mostram como o MIAR AI/FOOD transforma a rotina empresarial.
                        </motion.p>
                    </div>
                ...
                <div className="w-full max-w-[1600px] mx-auto px-6 md:px-12 pt-16 pb-32 space-y-16 relative">
                    <div className="flex items-center justify-between border-b border-border/50 pb-8">
                        <div className="space-y-1">
                            ...
                            <p className="text-muted-foreground/60 font-medium uppercase tracking-widest text-xs">Artigos • Notícias • Guias Práticos de Gestão</p>
                        </div>
                        <Link href="/conteudos" className="hidden md:flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-muted-foreground/40 hover:text-foreground transition-colors group">
                            ...
                            <div className="flex gap-8 md:gap-24 justify-center pt-12 pb-32 min-h-[600px] items-start relative">
                                <AnimatePresence mode="popLayout" initial={false}>
                                    {getVisibleBlogs().map((blog, index) => (
                                        ...
                                        >
                                            <Link href={`/conteudos/${blog.slug}`} className="block relative z-10 group/book">
                    ...
                    <div className="flex justify-center md:hidden pt-8">
                        <Link href="/conteudos" className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-muted-foreground/40 hover:text-foreground transition-colors group">
                            Ver Todos os Conteúdos
