...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    const title = post?.title || localPost?.title || 'Conteúdo HappyCash';
    const description = post?.excerpt || localPost?.excerpt || '';
    
    return {
        title: `${title} | HappyCash ERP`,
        description,
        keywords: [title, 'Gestão', 'HappyCash', 'ERP'],
    };
}
...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const article = post ? {
        ...
        author: { name: post.author || 'Equipe HappyCash' },
        ...
        <main className="min-h-screen bg-background pb-24 pt-32">
            ...
            <div className="container max-w-7xl mx-auto px-6 mb-12">
                <div className="relative w-full aspect-[21/9] md:aspect-[2/1] rounded-3xl overflow-hidden border border-border/40 shadow-2xl">
                    <Image
                        src={article.image || '/telas/tutorialinicial.webp'}
                        alt={article.title}
                        fill
                        sizes="100vw"
