...
export default function AchievementsPage() {
    ...
    const stats = useMemo(() => {
        ...
        const tutorials = portfolioData.achievements.filter(a => a.type?.toLowerCase() === 'tutorial').length;
        const guias = portfolioData.achievements.filter(a => a.type?.toLowerCase() === 'guia').length;
        const manuais = portfolioData.achievements.filter(a => a.type?.toLowerCase() === 'manual').length;
        return { total, tutorials, guias, manuais };
    }, []);
                    ...
                    >
                        ...
                        >
                            <motion.div variants={staggerItem}>
                                <NavItem label="Todos os Guias" active={activeCategory === 'all'} onClick={() => setActiveCategory('all')} count={getCategoryCount('all')} isCollapsed={isSidebarCollapsed} />
                            </motion.div>
                            <motion.div variants={staggerItem}>
                                <NavItem label="Começando" active={activeCategory === 'começando no happycash'} onClick={() => setActiveCategory('começando no happycash')} count={getCategoryCount('começando no happycash')} isCollapsed={isSidebarCollapsed} />
                            </motion.div>
                            <motion.div variants={staggerItem}>
                                <NavItem label="PDV & Caixa" active={activeCategory === 'pdv'} onClick={() => setActiveCategory('pdv')} count={getCategoryCount('pdv')} isCollapsed={isSidebarCollapsed} />
                            </motion.div>
                            <motion.div variants={staggerItem}>
                                <NavItem label="Estoque" active={activeCategory === 'estoque'} onClick={() => setActiveCategory('estoque')} count={getCategoryCount('estoque')} isCollapsed={isSidebarCollapsed} />
                            </motion.div>
                            <motion.div variants={staggerItem}>
                                <NavItem label="Financeiro" active={activeCategory === 'financeiro'} onClick={() => setActiveCategory('financeiro')} count={getCategoryCount('financeiro')} isCollapsed={isSidebarCollapsed} />
                            </motion.div>
                            <motion.div variants={staggerItem}>
                                <NavItem label="Clientes" active={activeCategory === 'clientes e fiado'} onClick={() => setActiveCategory('clientes e fiado')} count={getCategoryCount('clientes e fiado')} isCollapsed={isSidebarCollapsed} />
                            </motion.div>
                            <motion.div variants={staggerItem}>
                                <NavItem label="Configurações" active={activeCategory === 'configurações'} onClick={() => setActiveCategory('configurações')} count={getCategoryCount('configurações')} isCollapsed={isSidebarCollapsed} />
                            </motion.div>
                        </motion.nav>
                        ...
                        >
                            ...
                            <div className="flex gap-2 flex-wrap">
                                {[
                                    ...
                                    { icon: Medal, value: stats.tutorials },
                                    { icon: Award, value: stats.guias }
                                ].map((stat, i) => (
                                    <motion.div
                                        key={i}
                                        whileHover={{ scale: 1.05 }}
                ...
                >
                    <div className="max-w-5xl mx-auto px-6">
                        ...
                        >
                            ...
                            <p className="text-sm md:text-base text-muted-foreground/80 max-w-lg mx-auto mb-8 font-medium">
                                Não encontrou o que procurava? Nossa equipe está pronta para ajudar.
                            </p>
                            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                                <a
                                    href="https://wa.me/5512988918792"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="group relative px-6 py-3 bg-foreground text-background font-bold rounded-xl text-sm overflow-hidden transition-all hover:scale-105 active:scale-95 shadow-lg"
                                >
                        ...
                        >
                            <ErrorBoundary fallback={<div className="text-center opacity-50">Visuals Unavailable</div>}>
                                <FallingText
                                    text="Gestão Estoque Vendas PDV Fiado Caixa Relatórios NotaFiscal Configurações Equipe Lucro Resultados Suporte"
                                    highlightWords={['Gestão', 'Estoque', 'Vendas', 'Caixa', 'PDV']}
                                    trigger="scroll"
                                    gravity={0.8}
