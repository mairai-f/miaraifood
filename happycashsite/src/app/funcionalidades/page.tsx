...
export default function ProjectsPage() {
    ...
    const products = useMemo(() => {
        const telaImages = [
            ...
            "/telas/fechamentodecaixa.webp",
            "/telas/comandas.webp",
            "/telas/notafiscal.webp",
            "/telas/operaçoes.webp",
        ];

        const baseProducts = portfolioData.projects.map((p, i) => ({
            title: p.title,
            link: `/funcionalidades/${p.slug}`,
            thumbnail: p.image || telaImages[i % telaImages.length]
        }));
    ...
    const categories = [
        { id: 'All', label: 'Funcionalidades', icon: Globe },
                                    ...
                                    {filteredProjects.slice(0, visibleCount).map((project, index) => (
                                        <ProjectListItem
                                            ...
                                            onClick={() => {
                                                ...
                                                router.push(`/funcionalidades/${project.slug}`);
                                            }}
                                            index={index}
                    ...
                    {/* View All Button — Magnetic Fill-Invert */}
                    {
                        filteredProjects.length > 10 && (
                            ...
                            >
                                ...
                                >
                                    {visibleCount < filteredProjects.length ? 'Ver mais' : 'Ver menos'}
                                </MagneticFillButton>
                            </motion.div>
                        )
                    }
